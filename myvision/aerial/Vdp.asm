include "kh1000.inc"
include "Vram.inc"

cseg

SetVdpAddressDE: public SetVdpAddressDE
    ld a,e | ld (VdpAddress+2),a
    push af | pop af
    ld a,d | and 3fh | or 40h | ld (VdpAddress+2),a
    push af | pop af
ret


SetVdpAddressHL: public SetVdpAddressHL
    ld a,l | ld (VdpAddress+2),a
    push af | pop af
    ld a,h | and 3fh | or 40h | ld (VdpAddress+2),a
    push af | pop af
ret


VdpValues:
    defb 00h
    defb 0e2h
    defb Vram_PatternName/400h
    defb Vram_Color/40h
    defb Vram_PatternGenerator/800h
    defb Vram_SpriteAttribute/80h
    defb Vram_SpriteGenerator/800h
    defb 0
InitVdp: public InitVdp
    ld hl,VdpValues
    ld c,0
    ld b,8
    do
        ld a,(hl) | ld (VdpAddress+2),a
        inc hl
        ld a,c | or 80h | ld (VdpAddress+2),a
        inc c
    dwnz

    ld hl,Vram_SpriteAttribute
    ld bc,4*32
    ld a,0c0h
    call FILVRM
ret


WRTVRM: public WRTVRM
    di
        push af
            call SetVdpAddressHL
        pop af
        ld (VdpAddress),a
    ei
ret


RDVRM: public RDVRM
    di
        ld a,l | ld (VdpAddress+2),a
        push af | pop af
        ld a,h | and 3fh | ld (VdpAddress+2),a
        push af | pop af
        ld a,(VdpAddress)
    ei
ret


LDIRVM: public LDIRVM
    di
        call SetVdpAddressDE
        do
            ; push af | pop af
            ld a,(hl) | inc hl
            ld (VdpAddress),a
            ; push af | pop af
            dec bc
            ld a,c
            or b
        while nz | wend
    ei
ret

FILVRM: public FILVRM
    push de
        ld e,a
        di
            call SetVdpAddressHL
            do
                ld a,e | ld (VdpAddress),a
                dec bc
                ld a,c
                or b
            while nz | wend
        ei
    pop de
ret

WRTVDP: public WRTVDP
    push af
        di
            ld a,b
            ld (VdpAddress+2),a
            ld a,c
            or 80h
            ld (VdpAddress+2),a
        ei
    pop af
ret
