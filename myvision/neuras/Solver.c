...
void InitSolver(byte x, byte y) 
{
    ...
    ShowSprite(Sprite, SolverX, SolverY, pDirection->pattern, Color);
}
...
static bool Move(ptr<Direction> pNewDirection) 
{
    ...
    if (TestMap2(newX, newY)) {
        ...
        ShowSprite(Sprite, SolverX, SolverY, pDirection->pattern + seq, Color);
        return true;
    }
    return false;
}
...
void EndSolver()
{
    ...
    i = 0;
    repeat(20) {
        ShowSprite(Sprite, SolverX, SolverY, RotationPatterns[i & 3], Color);
        ++i;
        // EraseSprites();
        // DrawSprites();
        // VVramToVram();
