...
void InitMonsters(ptr<byte> pPosition, byte count) 
{
    ...
    while (count != 0 && i < MonsterCount) {
        ...
        ShowSprite(sprite, pMonster->x, pMonster->y, Pattern_Monster_Left, Color);
        ++pMonster;
        ++pPosition;
        ++sprite;
        --count;
    ...
    for (pMonster: Monsters) {
        if (pMonster->status == Monster_Status_Normal) {
            ...
            if (TestMap2(newX, newY)) {
                ...
                pMonster->y = newY;
                seq = ((newX + newY + 4) >> 3) & 1;
                ShowSprite(sprite, newX, newY, pDirection->pattern + seq, Color);
            ...
            ShowSprite(sprite, pMonster->x, pMonster->y, pattern, Color);
            --pMonster->stopCount;
