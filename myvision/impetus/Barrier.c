...
constexpr byte RangeY = (VramHeight - 1) * CoordRate;
...
void DrawBarriers(word page)
{
    ptr<Barrier> pBarrier;
    for (pBarrier : Barriers) {
        if (pBarrier->newY < VramHeight) {
            byte c, count;
            word vram;
            c = Char_Barrier + (((pBarrier->y << 1) + 4) & 7);
            count = pBarrier->length;
            vram = page + VramOffset((pBarrier->left >> CoordShift) + 1, pBarrier->newY);
            do {
