...
static void Show()
{
    ShowSprite(Sprite_Fighter, FighterX, FighterY, Pattern_Fighter, Color);
}
...
void MoveFighter()
{
    ...
    if ((key & Keys_Right) != 0 && FighterX < (VramWidth - 2) * CoordRate) {
        ++FighterX;
    }
    if ((key & Keys_Up) != 0 && FighterY > CoordRate * 2) {
        --FighterY;
    }
    if ((key & Keys_Down) != 0 && FighterY < (VramHeight - 2) * CoordRate) {
        ++FighterY;
    }
    if (ReviveCount > 0) {
        --ReviveCount;
