...
static const byte[] MidChars = {
    ...
    Char_Wall_RightBottom, Char_Wall_LeftBottom, Char_Soil+1, Char_Soil, 
...
{
    byte c;
    c = Pattern_Man + charOffset + ((man.status & Movable_Direction_Mask) << 2);
    c += ((man.x + man.y) >> (CoordShift - 2)) & 4;
    ShowSprite(&man, c, Color);
}
