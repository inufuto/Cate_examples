...
static const Direction[] Directions = {
    { -1, 0, Char_Knife_Left - Char_Knife },
    { 1, 0, Char_Knife_Right - Char_Knife },
    { 0, -1, Char_Knife_Up - Char_Knife },
    { 0, 1, Char_Knife_Down - Char_Knife },
};
...
static void Show(ptr<Knife> pKnife)
{
    ...
    if ((status & (Visible | Moving)) != 0) {
        byte pattern = Char_Knife + ((status & DirectionMask) << 2);
        ShowSprite(pKnife, pattern);
    }
    else {
...
void StartKnife()
{
    ...
    for (pKnife: Knives) {
        ...
        if ((status & Movable_Live) == 0) {
            ...
            if (dy > 0) y += CoordRate;
            pKnife->y = y;
            pKnife->status = Movable_Live | Moving | (pDirection->pattern >> 2);
            --HeldKnifeCount;
            Sound_Get();
            PrintHeldKnives();                    
            Hit(pKnife);
