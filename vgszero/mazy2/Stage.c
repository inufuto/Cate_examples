...
void InitFloor()
{
    ...
    }
    {
        ...
        pVram -= (1 + 3 * ColumnCount) * VramStep;
        repeat (RowCount) {
            repeat (4) {
                pVram = Put(pVram, Char_Space);
                pVram += VramRowSize - 1 * VramStep;
            }
            pVram += -VramRowSize * 4;
            repeat (ColumnCount) {
                byte cell = *pMap;
                if ((cell & Cell_Visible) != 0) {
                    pVram = Put(pVram, Char_Wall);
                    {
                    ...
                    pVram += 1 * VramStep - VramRowSize * 2;
                    ...
                    pVram = Put(pVram, Char_Wall);
                    pVram += -VramRowSize * 3 - 1 * VramStep;
                }
                else {
                    pVram += VramRowSize + 1 * VramStep;
                    repeat (3) {
                        ...
                        pVram += VramRowSize - 3 * VramStep;
                    }
                    pVram += 2 * VramStep - VramRowSize * 4;
                }
                ++pMap;
            }
            pVram += VramRowSize * 3 - 30 * VramStep;
        }
    }
    ShowMonsters();
    ShowKnives();
