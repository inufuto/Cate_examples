    ...
    CopyMemory(CellMap, pStage->bytes, ColumnCount / ColumnsPerByte * RowCount);
    {
        ...
        constexpr byte Wall_All = 0x0f;
        {
            ...
            repeat (RowCount) {
                ...
                repeat (ColumnCount / ColumnsPerByte) {
                    ...
                    repeat (ColumnsPerByte) {
                        if ((cell & Cell_BottomWall) != 0) {
                            Put(pVram + VramRowSize * 2, Char_Wall | Wall_All);
                            Put(pVram + VramRowSize * 2 + 1 * VramStep, Char_Wall | Wall_All);
                        }
                        pVram += 2 * VramStep;
                ...
                pVram += VramRowSize * 2 + 1 * VramStep;
            }
        }
        {
            ptr<byte> pVram = VramBack + VramRowSize * StatusHeight;
            ...
            for (y = 0; y < VramHeight - StatusHeight; ++y) {
                ...
                for (x = 0; x < VramWidth; ++x) {
                    byte c = pVram[0];
                    if (x > 0 && pVram[-1 * VramStep] != Char_Space) {
                        c &= ~Wall_Left;
                    }
                    if (pVram[1 * VramStep] != Char_Space) {
                        c &= ~Wall_Right;
                    }
                    if (y > 0 && pVram[-VramRowSize] != Char_Space) {
                        c &= ~Wall_Top;
                    }
                    if (y < VramHeight - StatusHeight - 1 && pVram[+VramRowSize] != Char_Space) {
                        c &= ~Wall_Bottom;
                    }
                    pVram = Put(pVram, c);
                }
        ...
        }
        {
            ...
            repeat (RowCount) {
                ...
                pVram += VramRowSize * 2 + 2 * VramStep;
            }
            PrintFoodCount();
        }
    }
