...
constexpr byte Color1 = 4;
constexpr byte Color2 = 11;
// constexpr byte MaxCount = Sprite_Chaser - Sprite_Ghost;
Movable[MaxGhostCount] Ghosts;
...
ptr<byte> InitGhosts(ptr<byte> pStage)
{
    ...
    return InitMonsters(Ghosts, MaxGhostCount, Sprite_Ghost, pStage);
}


static void Draw(ptr<Movable> pGhost)
{
    byte status, c, xy, color;
    status = pGhost->status;
    c = Pattern_Ghost + ((status & Movable_Direction_Mask));
    xy = pGhost->x + pGhost->y;
    c += (xy >> (CoordShift)) & 1;
    if ((status & Ghost_Status_Throgh) != 0) {
        c += 8;
    }
    ShowSprite(pGhost->sprite, pGhost->x, pGhost->y, c);
}
...
void StartGhosts()
{
    StartMonsters(Ghosts, MaxGhostCount, pGhostPositions);
    {
        ptr<Movable> pGhost;
        for (pGhost: Ghosts) {
...
{
    HitMonsterInArray(Ghosts, MaxGhostCount, pMovable);
}


bool AdjacentGhost(byte sprite, byte x, byte y)
{
    return AdjacentOtherInArray(Ghosts, MaxGhostCount, sprite, x, y);
}