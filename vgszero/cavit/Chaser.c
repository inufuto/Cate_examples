...
constexpr byte Color = 10;
// constexpr byte MaxCount = Sprite_Rock - Sprite_Chaser;
Movable[MaxChaserCount] Chasers;
static ptr<byte> pChaserPositions;

ptr<byte> InitChasers(ptr<byte> pStage)
{
    pChaserPositions = pStage;
    return InitMonsters(Chasers, MaxChaserCount, Sprite_Chaser, pStage);
}


static void Draw(ptr<Movable> pChaser)
{
    ShowSprite(pChaser->sprite, pChaser->x, pChaser->y, Pattern_Chaser);
}
...
void StartChasers()
{
    StartMonsters(Chasers, MaxChaserCount, pChaserPositions);
    {
        ptr<Movable> pChaser;
        for (pChaser: Chasers) {
...
{
    HitMonsterInArray(Chasers, MaxChaserCount, pMovable);
}


bool AdjacentChaser(byte sprite, byte x, byte y)
{
    return AdjacentOtherInArray(Chasers, MaxChaserCount, sprite, x, y);
}