...
void PrintStatus() 
{
    ...
    {
        word vram;
        vram = Vram + VramRowSize * 21 + 32 * VramStep;
        if (RemainCount > 1) {
            byte i;
            i = RemainCount - 1;
            if (i > 2) {
...
{
    word vram;
    vram = Vram + VramRowSize * 19 + 32 * VramStep;
...
void Title()
{
    ...
    PrintS(Vram + VramRowSize * 19 + 9 * VramStep, "PUSH SHIFT KEY");
    PrintS(Vram + VramRowSize * 20 + 9 * VramStep, "OR CTRL KEY");
