...
void PrintScore()
{
    ...
    vram = PrintNumber5(Vram + VramRowSize * 2 + (40 - 6) * VramStep, Score);
    PrintC(vram, '0');

    vram = PrintNumber5(Vram + VramRowSize * 6 + (40 - 6) * VramStep, HiScore);
    PrintC(vram, '0');
}

void PrintStage()
{
    PrintByteNumber2(Vram + VramRowSize * 8 + (40 - 2) * VramStep, CurrentStage + 1);
}
