...
void PrintStatus() 
{
    ...
    PrintS(Vram + VramRowSize * 3 + 24 * VramStep, "STAGE");
    PrintByteNumber2(Vram + VramRowSize * 4 + 28 * VramStep, CurrentStage + 1);
...
void PrintScore()
{
    ...
    pVram = PrintNumber5(Vram + VramRowSize * 1 + 24 * VramStep, Score);
    PrintC(pVram, '0');
...
{
    ptr<byte> pVram;
    pVram = Vram + VramRowSize * 6 + 24 * VramStep;
    if (RemainCount >= 3 + 1) {
