...
constexpr byte Char_Remain = Char_Solver;
...
void PrintHeldCard() 
{
    if (pHeldCard != nullptr) {
        ...
        Put2C(Vram + VramRowSize * 10 + 25 * VramStep, pattern);
    }
    else {
        Erase2(Vram + VramRowSize * 10 + 25 * VramStep);
    }
}
...
//     word top, bottom;
//     top = Vram;
//     bottom = Vram + VramRowSize * (VramHeight - 1);
//     repeat(StageWidth) {
