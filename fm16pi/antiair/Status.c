...
void PrintScore()
{
    ...
    vram = PrintNumber5(Vram + VramRowSize * 2 + LeftEnd - 6 * VramStep, Score);
    PrintC(vram, '0');

    vram = PrintNumber5(Vram + VramRowSize * 6 + LeftEnd - 6 * VramStep, HiScore);
    PrintC(vram, '0');
}

void PrintStage()
{
    PrintByteNumber2(Vram + VramRowSize * 8 + LeftEnd - 2 * VramStep, CurrentStage + 1);
}
