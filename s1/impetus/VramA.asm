...
DrawGround_: public DrawGround_
    pshs a,b,x,y
        ...
        ldx #Ground_
        do
            lda #StageWidth/TileSize | sta rowCount
            do
                lda ,x+
                pshs x
                    ldb #TileSize*TileSize*2 | mul
                    addd #Tiles_
                    tfr d,x
                    lda coff
                    leax a,x
                    ...
                    do
                        ...
                        dec yCount
                    while ne | wend
                    leay TileSize-VramRowSize*TileSize,y
                    lda yPos | suba #TileSize | sta yPos
                puls x
                dec rowCount
            while ne | wend
            leay VramRowSize*TileSize-StageWidth,y
            lda yPos | adda #TileSize | sta yPos
            cmpa #StageHeight
        while cs | wend
    puls a,b,x,y
rts
...
dseg
FortWY:
    defs CharHeight*PlaneCount*6*7
cseg
InitFortChars:
    ldy #FortPattern
    ldx #CRam+CharHeight*Char_Fort0
    ldb #6*6
    bsr MakePatternColor
