...
static void Draw(ptr<Bullet> pBullet)
{
    if ((pBullet->status & Bullet_Live_Mask) != 0) {
        ShowSprite(pBullet->sprite, pBullet->x, pBullet->y + StageTop * CoordRate, 
            Pattern_Bullet + (pBullet->status & Bullet_Pattern_Mask));
    }
    else {
...
static void Start(ptr<Bullet> pBullet, byte x, byte y, byte sprite, byte direction, byte side)
{
    static const byte[] Sizes = {
        SizeInCoord * 1 - CoordRate / 2, SizeInCoord * 2 - CoordRate / 2,
        SizeInCoord * 1 - CoordRate / 2, SizeInCoord * 2 - CoordRate / 2,
        SizeInCoord * 2 - CoordRate / 2, SizeInCoord * 1 - CoordRate / 2,
        SizeInCoord * 2 - CoordRate / 2, SizeInCoord * 1 - CoordRate / 2,
    };
