...
void VPut6(sbyte x, sbyte y, byte c) 
{
...
bool HitFort(byte x, byte y, byte size) 
{
    ...
    if (pFort != nullptr) {
        ...
        if (pFort->life == 0) {
            ...
            AddScore(50);
            DrawFortRader(pFort->x - Size, pFort->y - Size, false);
        }
        else {
            Sound_SmallBang();
            StartBang(x, y, false);
...
void DrawFortsOnRader()
{
    ...
    for (pFort : Forts) {
        if ((pFort->flags & (Flag_Live | Flag_Visible)) == 0 || pFort->life <= 0) continue;
        DrawFortRader(pFort->x - Size, pFort->y - Size, true);
    }
}
