...
constexpr byte Ground_TimeMask = 0xf0;
...
void DrawBackground()
{
    FillMemory(Vram + VramRowSize * StatusHeight, VramRowSize * (WindowHeight - 2), 0);

    ptr<byte> pUpper = Vram + VramRowSize * (StatusHeight + WindowHeight - 2);
    ptr<byte> pLower = Vram + VramRowSize * (StatusHeight + WindowHeight - 2 + 1);
    bool right = false;
    ptr<byte> pGround = Ground;
    repeat (WindowWidth) {
        byte upper, lower;
        byte b = *pGround; ++pGround;
        byte bomb = b & Ground_BombMask;
        if ((b & Ground_LeftBomb) != 0) {
            upper = 0x03;
            lower = 0xf3;
        }
        else if ((b & Ground_RightBomb) != 0) {
            upper = 0xac;
            lower = 0x3c;
        }
        else if ((b & Ground_LeftBlock) != 0) {
            upper = 0xa9;
            lower = 0x9a;
        }
        else if ((b & Ground_RightBlock) != 0) {
            upper = 0xa6;
            lower = 0x6a;
        }
        else {
        ...
        pUpper = Put(pUpper, upper, right);
        pLower = Put(pLower, lower, right);
        right = !right;
    }
}