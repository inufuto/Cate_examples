...
constexpr byte Count = Sprite_Increaser - Sprite_Monster;
...
static void Show(ptr<Actor> pMonster)
{
    ...
    pattern = ((status & Actor_DirectionMask) >> 1) + Pattern_Monster;
        ...
        if ((status & Actor_Live) != 0) {
            // if ((status & Monster_Weak) == 0) {
                ...
                if (pMonster->dx != 0 || pMonster->dy != 0) {
                    ...
                    // byte seq = ((pMonster->_.x + pMonster->_.y + CoordRate / 2) >> CoordShift) & 1;
