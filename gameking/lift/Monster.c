...
{
    byte c = pMonster->c;// + ((pMonster->status & Actor_SeqMask));
...
{   
    ...
    if (dx < 0) {
        pMonster->c = Pattern_Monster_Left;
    }
    else if (dx > 0) {
        pMonster->c = Pattern_Monster_Right;
    }
    pMonster->dx = dx;
...
void MoveMonsters()
{
    ...
    for (pMonster: Monsters) {
        ...
        if (pMonster->dx != 0 && (pMonster->status & WaitBit) == 0) {
            ...
            // byte seq = ((pMonster->_.x + CoordRate / 2) >> CoordShift) & 1;
