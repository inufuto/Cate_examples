...
static void Show(ptr<Monster> pMonster)
{
    ...
    if ((pMonster->_.status & Movable_Pushing) != 0) {
        pattern = ((pMonster->_.status & Movable_PatternMask) << 1) + Pattern_Monster + 4 * 8;
    }
    else {
        pattern = ((pMonster->_.status & Movable_PatternMask) << 2) + Pattern_Monster;
    }
    ShowSprite(&pMonster->_, pattern, Color);
}
