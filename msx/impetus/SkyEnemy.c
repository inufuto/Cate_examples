...
constexpr byte Count = Sprite_GroundEnemy - Sprite_SkyEnemy;
...
static const byte[] Colors = { 9, 13, 10 };
static const word[] Points = { 4, 6, 10 };
static const byte[] Paterns = { 
    Pattern_SkyEnemy + 0, Pattern_SkyEnemy + 4, Pattern_SkyEnemy + 4 * (1 + 8) 
};
    ...
    case Type_Smart: 
        {
            ...
            pEnemy->pattern = Pattern_SkyEnemy + 4 + (pEnemy->direction << 2);
...
{
    ShowSprite(pEnemy->sprite, pEnemy->x, pEnemy->y, pEnemy->pattern, Colors[pEnemy->type]);
}
