include "BIOS.inc"
include "Vram.inc"
include "Chars.inc"

ext MonoPattern, ColorPattern

cseg
ColorSource:
defb 30, 0f0h ; Ascii
defb 15, 0a0h ; Logo
defb 2, 0a0h ; Ladder
defb 4, 0b0h ; Item
defb 4, 70h ; Debri
defb 4, 0f0h ; Remain
defb 0
cseg
InitVram: public InitVram
    call INIGRP
    xor a
    ld (BDRCLR),a
    call CHGCLR

    call ClearScreen_

    ld hl,MonoPattern
    ld de,Vram_PatternGenerator and 3fffh
    ld bc,Char_End*CharHeight
    call LDIRVM

    ld hl,Vram_Color and 3fffh
    ld de,ColorSource
    do
        ld a,(de) | or a
    while nz
        ld c,a
        inc de
        ld a,(de) | inc de
        do
            ld b,CharHeight
            do
                push af
                    call WRTVRM
                pop af
                inc hl
            dwnz
            dec c
        while nz | wend
    wend

    ld de,ColorPattern
    ld hl,(Vram_PatternGenerator+CharHeight*Char_Color) and 3fffh
    ld c,Char_End-Char_Color
    do
        push hl
            ld b,CharHeight
            do
                ld a,(de) | inc de
                call WRTVRM
                inc hl
            dwnz
        pop hl
        set 5,h
            ld b,CharHeight
            do
                ld a,(de) | inc de
                call WRTVRM
                inc hl
            dwnz
        res 5,h
        dec c
    while nz | wend

    ld hl,(Vram_PatternGenerator+800h) and 3fffh
    exx
        ld hl,Vram_PatternGenerator and 3fffh
    exx
    ld c,2
    do
        ld b,2
        do
            ld de,800h
            do
                exx
                    call RDVRM
                    inc hl
                exx
                call WRTVRM
                inc hl
                dec de
                ld a,e
                or d
            while nz | wend
        dwnz
        ld hl,Vram_Color+800h
        exx
            ld hl,Vram_Color
        exx
        dec c
    while nz | wend
ret

; void ClearScreen();
cseg
ClearVram:
    inc c
    out (c),l
    out (c),h
    dec c
    ld b,0
    ld hl,300h
    do
        out (c),b
        dec hl
        ld a,l | or h
    while nz | wend
ret
ClearScreen_: public ClearScreen_
    push af | push hl | push bc
        ld a,(VdpOutPort) | ld c,a

        ld hl,Vram_PatternName
        call ClearVram
    pop bc | pop hl | pop af
ret


; word VramAddress(byte x, byte y);
VramAddress_: public VramAddress_
    push de
        ld l,e
        ld h,0
        add hl,hl ;*2
        add hl,hl ;*4
        add hl,hl ;*8
        add hl,hl ;*16
        add hl,hl ;*32
        ld e,a
        ld d,0
        add hl,de
        ld de,Vram+VramRowSize*StatusHeight
        add hl,de
    pop de
ret


WriteHlA:
    di
        inc c
        out (c),l
        out (c),h
        dec c
        out (c),a
    ei
ret

; word Put(word vram, byte c);
cseg
Put_: public Put_
    push af | push bc
        ld a,(VdpOutPort) | ld c,a
        ld a,e
        call WriteHlA
        inc hl
    pop bc | pop af
ret


; word PrintC(word vram, byte c);
cseg
CharTable:
    defb " -0123456789ACEFGHIKMNOPRSTUVY"
PrintC_: public PrintC_
    push af | push bc
        push hl
            ld a,e
            ld bc,30*256
            ld hl,CharTable
            do
                cp (hl)
                jr z,PrintC_break
                inc hl
                inc c
            dwnz
            ld c,0
            PrintC_break:
        pop hl
        ld b,c
        ld a,(VdpOutPort) | ld c,a
        ld a,b
        call WriteHlA
        inc hl
    pop bc | pop af
ret
