...
static ptr<byte> DrawMovablePanel(ptr<byte> pVVram, byte b, byte edge)
{
    {
        ...
        pVVram = VPut(pVVram, Char_MovablePanel + 0);
        repeat (2) {
            pVVram = VPut(pVVram, *pChars); ++pChars;
        }
        pVVram = VPut(pVVram, Char_MovablePanel + 1);
        pVVram += VVramWidth - 4;
    }
    {
    ...
    }
    {
        ...
        pVVram = VPut(pVVram, Char_MovablePanel + 2);
        repeat (2) {
            pVVram = VPut(pVVram, *pChars); ++pChars;
        }
        pVVram = VPut(pVVram, Char_MovablePanel + 3);
        pVVram += -VVramWidth * 3;
    }
    return pVVram;
}
...
static ptr<byte> DrawPanel(ptr<byte> pVVram, byte b, byte edge)
{
    // if ((b & Panel_Movable) != 0) {
        return DrawMovablePanel(pVVram, b, edge);
    // }
    // else {
    //     return DrawFixedPanel(pVVram, b, edge);
    // }
}
...
{
    byte y = PanelY;
    ptr<byte> pVVram = VVramPtr(PanelX, y);
    if (column != 0) {
    ...
    {
        byte edge = Edge(pMap, column, row);
        DrawMovablePanel(pVVram, *pMap, edge);
    }
}
...
void MovePanel()
{
    ...
    if ((clock & CoordMask) == 0 && MovingPanel != 0) {
        ...
        if (PanelDirection == Direction_Left) {
            ptr<byte> pVVram = VVramPtr(PanelX - 1, PanelY);
            pVVram = DrawMovablePanel(pVVram, MovingPanel, 0);
            EraseV(pVVram);
            --PanelX;
            dx = -CoordRate;
            dy = 0;
        ...
        if (PanelDirection == Direction_Right) {
            ptr<byte> pVVram = VVramPtr(PanelX, PanelY);
        ...
        if (PanelDirection == Direction_Up) {
            ptr<byte> pVVram = VVramPtr(PanelX, PanelY - 1);
            pVVram = DrawMovablePanel(pVVram, MovingPanel, 0);
        ...
        if (PanelDirection == Direction_Down) {
            ...
            DrawMovablePanel(pVVram, MovingPanel, 0);
            ++PanelY;
            dx = 0;
            dy = CoordRate;
            goto moved;
