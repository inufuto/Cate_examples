...
{
    byte status = pCar->status;
    byte pattern = (status & Actor_PatternMask) + Pattern_Car;
    pattern = pattern | (status & Car_OverLeft);
...
void LooseCar()
{
    ...
    byte over = pLostCar->status & Car_OverLeft;
    byte i = 0;
    do {
        ShowSprite(&pLostCar->_, patterns[i & 3] | over);
        // UpdateSprites();
        Sound_Loose();
        ++i;
    } while (i < 8);
