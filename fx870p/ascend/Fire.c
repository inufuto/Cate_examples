...
constexpr byte Char_Fire = 0;
constexpr byte Char_Fire_Left = Char_Fire;
constexpr byte Char_Fire_Right = Char_Fire + 1 * 4;
        ...
        if ((pFire->status & Actor_Live) != 0) {
            ...
            if (pFire->_.y > topY && pFire->_.y - topY >= WindowHeight) {
                ...
                if ((pFire->status & Item_1Up) != 0) {
                    ...
                    ShowSprite(&pFire->_, Chars);
                }
                else if ((pFire->status & Item_Power) != 0) {
                    const byte[] Chars = {  0x30, 0xc0, 0x10, 0x40, };
                    ShowSprite(&pFire->_, Chars);
                }
                else {
                    ShowEnemy(pFire);
