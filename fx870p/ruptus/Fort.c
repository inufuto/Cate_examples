...
{
    const byte[] Chars = {
        0x10, 0xd0, 0xe0, 0xb0, 0x70, 0x40, 0xb0, 0x90,
        0xa0, 0xa0, 0x60, 0xe0, 0xe0, 0xc0, 0x60, 0x90,
        0x30, 0xb0, 0xd0, 0xc0, 0x90, 0x60, 0x30, 0x70,
        0x70, 0x60, 0x50, 0x50, 0x90, 0xd0, 0x20, 0xe0,
        0xd0, 0x70, 0xb0, 0x80,
    };
    ptr<Fort> pFort;
...
bool HitFort(byte x, byte y, byte size) 
{
    ...
    if (pFort != nullptr) {
        ...
        if (pFort->life == 0) {
            ...
            AddScore(50);
            DrawFighterOnRader();
            DrawFortOnRader(pFort->x - HalfSize, pFort->y - HalfSize, false);
            DrawFighterOnRader();
        }
        else {
            if (size == 0) {
...
void DrawFortsOnRader()
{
    ...
    for (pFort : Forts) {
        if ((pFort->flags & (Flag_Live | Flag_Visible)) == 0 || pFort->life <= 0) continue;
        DrawFortOnRader(pFort->x - HalfSize, pFort->y - HalfSize, true);
    }
}
