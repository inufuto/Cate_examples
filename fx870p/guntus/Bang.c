...
static byte Show(byte x, byte y, byte sprite, byte pattern)
{
    if (sprite < NextSprite) {
        ...
        if (x < RangeX && y < RangeY) {
            ShowSprite(sprite, x, y, Chars + pattern);
            return sprite + 1;
        }
    }
    return sprite;
...
void UpdateBangs()
{
    ...
    for (pBang : Bangs) {
        ...
        if (mode == Status_Large_Large) {
            sprite = Show(pBang->x - Size, pBang->y - Size, sprite, 4);
            sprite = Show(pBang->x + Size, pBang->y - Size, sprite, 8);
            sprite = Show(pBang->x - Size, pBang->y + Size, sprite, 12);
            sprite = Show(pBang->x + Size, pBang->y + Size, sprite, 16);
        }
        else {
            sprite = Show(pBang->x, pBang->y, sprite, 0);
        }
