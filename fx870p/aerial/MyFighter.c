...
constexpr byte Char_Fighter_Right = 0;
constexpr byte Char_Fighter_RightDown = 4;
constexpr byte Char_Fighter_RightUp = 8;
...
void MoveMyFighter()
{
    ...
    if ((MyFighter.clock & 1) == 0) {
        ...
        if ((key & Keys_Up) != 0 && MyFighter.y > 0) {
            --MyFighter.y;
            MyFighterDy = -1;
            pattern = Char_Fighter_RightUp;
        }
        if ((key & Keys_Down) != 0 && MyFighter.y < RangeY) {
            ++MyFighter.y;
            MyFighterDy = 1;
            pattern = Char_Fighter_RightDown;
        }
