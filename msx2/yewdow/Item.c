...
{
    Put2CXY(pItem->x, pItem->y + StatusHeight, ((index & 0xfe) << 1) + Char_Item);
}
...
void HitItems(byte x, byte y)
{
    ...
    if (cell == Cell_Item) {
        ...
        for (pItem : Items) {
            byte iy;
            iy = pItem->y & ~Hidden;
            if (pItem->x != InvalidX && pItem->x >> 1 == x && iy >> 1 == y) {
                byte type;
                type = index >> 1;
                ...
                StartPoint(pItem->x << CoordShift, iy << CoordShift, Rate);
                EraseCell(x, y);
                pItem->x = InvalidX;
                --ItemCount;
                Sound_Hit();
