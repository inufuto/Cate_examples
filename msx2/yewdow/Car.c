...
static void Show(ptr<Actor> pCar)
{
    byte pattern, status;
    status = pCar->status;
    pattern = ((status & Actor_PatternMask) << 1) + Pattern_Car;
    pattern |= status & Car_OverLeft;
    ShowSpriteC(&pCar->_, pattern);
}
...
void LooseCar()
{
    ...
    byte i, shift;
    shift = pLostCar->status & Car_OverLeft;
    i = 0;
    do {
        ShowSpriteC(&pLostCar->_, patterns[i & 3] | shift);
        UpdateSprites();
        Sound_Loose();
        ++i;
    } while (i < 8);
