...
constexpr byte FireSpriteColor = 4;
constexpr byte ItemSpriteColor = 7;
        ...
        if (
            ...
            for (pFire : Fires) {
                if ((pFire->status & Actor_Live) == 0) {
                    ...
                    ShowEnemy(pFire, FireSpriteColor);
                    spawnCount -= Interval;
                    return;
                }
            }
        ...
        if ((pFire->status & Actor_Live) != 0) {
                ...
                if (IsOnFloor(pFire->_.y)) {
                    if ((pFire->_.x & 1) == 0 && (
                        ...
                        if (pFire->dx < 0) {
                            if (!CanMove(pFire, -1)) {
                                pFire->dx = 1;
                                pFire->pattern = Pattern_Fire_Right;
                            }
                            else {
                                pFire->pattern = Pattern_Fire_Left;
                            }
                        }
                        else if (pFire->dx > 0) {
                            if (!CanMove(pFire, 1)) {
            ...
            if (pFire->_.y > topY && pFire->_.y - topY >= WindowHeight) {
                ...
                if ((pFire->status & Item_1Up) != 0) {
                    ShowSpriteM(&pFire->_, Pattern_Item_1Up, ItemSpriteColor);
                }
                else if ((pFire->status & Item_Power) != 0) {
                    ShowSpriteM(&pFire->_, Pattern_Item_Power, ItemSpriteColor);
                }
                else {
                    ShowEnemy(pFire, FireSpriteColor);
                }
