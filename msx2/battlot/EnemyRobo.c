...
constexpr byte Count = (Sprite_Bang - Sprite_EnemyRobo) / 2;
...
static void Draw(ptr<Robo> pRobo)
{
    if ((pRobo->status & Robo_Status_Mask) != Robo_Status_None) {
        ShowSpriteC(pRobo->sprite, pRobo->x, pRobo->y + StageTop * CoordRate, 
            Pattern_EnemyRobo + ((pRobo->status & Robo_Pattern_Mask) << 1));
    }
    else {
...
byte  DecideDirection(ptr<Robo> pRobo)
{
    ...
    if (target == 0) {
        targetX = MyRobo.x >> CoordShift;
        targetY = MyRobo.y >> CoordShift;
    }
    else {
        targetX = MyFort.x;
        targetY = MyFort.y;
...
void MoveEnemyRobos()
{
    ...
    for (pRobo: EnemyRobos) {
        ...
        if (status != Robo_Status_None) {
            if ((pRobo->x & CoordMask) == 0 && (pRobo->y & CoordMask) == 0) {
                bool fire;
                byte wall;
                fire = false;
