...
static void InitStairs(ptr<byte> pStairs, byte type, byte count)
{
    while (count != 0) {
        ...
        byte column = b >> 4;
        byte row = b & 0x0f;
        word pMap = CellMapPtr(column, row);
        WriteVram(pMap, ReadVram(pMap) | type);
        --count;
    }
}


static bool SetCellVisible(word pMap)
{
    byte cell = ReadVram(pMap);
...
void InitFloor()
{
        ...
        for (row = 0; row < RowCount; ++row) {
            ...
            for (column = 0; column < ColumnCount;) {
                ...
                repeat (4) {
                    ...
                    source >>= 2;
                            _deb();
                    if (column == 0 || (ReadVram(pMap - 1) & Cell_RightWall) != 0) {
                        cell |= Cell_LeftWall;
                    }
                    if (row == 0 || (ReadVram(pMap - ColumnCount) & Cell_BottomWall) != 0) {
                        cell |= Cell_TopWall;
                    }
                    WriteVram(pMap, cell); ++pMap;
                    ++column;
                }
            }
            PollVSync();
    ...
    if (CurrentFloor == 0) {
        ...
        byte row = b & 0x0f;
        word pMap = CellMapPtr(column, row);
        WriteVram(pMap, ReadVram(pMap) | Cell_Goal);
        GoalX = ToCoord(column);
        GoalY = ToCoord(row);
    }
    {
        byte startColumn = ToGrid(ReadVram(Man + Movable_x));
        byte startRow = ToGrid(ReadVram(Man + Movable_y));
        word pMap = CellMapPtr(startColumn, startRow);
        WriteVram(pMap, ReadVram(pMap) | Cell_NextScan);
    }
    {
        bool changed;
        loop:
