...
static const byte[] Chars = {
    0xe0, 0xd0, 0xb0, 0x70, 0xb0, 0x90, 0xb0, 0x70,
	0x30, 0x20, 0xf0, 0xd0, 0x00, 0x00, 0x70, 0xd0,
	0x00, 0x00, 0x10, 0xb0, 0x10, 0x00, 0x00, 0x80,
	0x10, 0x20, 0x40, 0x80, 0x30, 0xa0, 0x50, 0xc0,
	0x60, 0x90, 0x90, 0x60,
};
...
static void Draw(ptr<FixedBlock> pFixedBlock, ptr<byte> pChars)
{
    ptr<byte> pVVram = VVramPtr(
        pFixedBlock->column << CellShift, 
        (pFixedBlock->row << CellShift) - TopMargin
    );
    repeat (2) {
        repeat (2) {
...
void DrawBlocks()
{
    ...
    for (pFixedBlock : FixedBlocks) {
        ...
        case Block_Destroying:
            {
                byte index = pFixedBlock->count >> CoordShift;
                Draw(pFixedBlock, Chars + 4 + (index << 2));
            }
            break;
        case Block_Restarting:
            {
                byte index = pFixedBlock->count >> CoordShift;
                Draw(pFixedBlock, Chars + 20 + (index << 2));
            }
            break;
        }
    }
