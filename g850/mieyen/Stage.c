...
void InitTrying()
{
    ...
    CopyMemory(CellMap, pStage->bytes, ColumnCount / ColumnsPerByte * RowCount);
    {
        ...
        constexpr byte Wall_All = 0x0f;
        {
            ...
            repeat (ColumnWidth * ColumnCount) {
                ...
                repeat (ColumnCount / ColumnsPerByte) {
                    ...
                    repeat (ColumnsPerByte) {
                        if ((cell & Cell_BottomWall) != 0) {
                            VPut(pVVram + VVramWidth * 2, Char_Wall);
                            VPut(pVVram + VVramWidth * 2 + 1, Char_Wall);
                        }
                        pVVram += 2;
                        if (column < ColumnCount - 1 && (cell & Cell_RightWall) != 0) {
                            VPut(pVVram, Char_Wall);
                            VPut(pVVram + VVramWidth, Char_Wall);
                        }
        ...
        {
            FoodCount = 0;
            ptr<byte> pVVram = VVramBack + VVramWidth;
            ptr<byte> pCell = CellMap;
            repeat (RowCount) {
                repeat (ColumnCount / ColumnsPerByte) {
                    byte cell = *pCell; ++pCell;
...
void SetCell(byte column, byte row, byte cell)
{
    ...
    ptr<byte> pVVram = VVramBack + VVramOffset(ToCoord(column) - 1, ToCoord(row));
    if ((cell & Cell_Dot) != 0) {
        VPut2C(pVVram, Char_Dot);
    }
