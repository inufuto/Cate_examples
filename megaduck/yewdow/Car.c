...
static void Show(ptr<Actor> pCar)
{
    ...
    ShowSprite(&pCar->_, pattern, (status & Car_OverLeft) != 0);
}
...
void MoveCars()
{
    ...
    for (pCar : Cars) {
        ...
        if (pCar->dx < 0 && pCar->_.x >= -CoordRate) {
...
void LooseCar()
{
    ...
    else if (x >= RangeX - CoordRate) {
        x = RangeX - CoordRate;
    }
    x = ScrollTarget(x);
    while (HScroll != x) {
        HScroll += Sign(x, HScroll);
        UpdateSprites();
        WaitTimer(1);
    }
    do {
        ShowSprite(&pLostCar->_, patterns[i & 3], (pLostCar->status & Car_OverLeft) != 0);
        UpdateSprites();
        Sound_Loose();
        ++i;
    } while (i < 8);
