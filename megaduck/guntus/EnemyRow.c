...
void DrawEnemyRows()
{
    ...
    leftCoord = (EnemyRowLeft >> CoordShift);
                            _deb();
    word vram = NextPage + ((word)leftCoord) + (VramRowSize * Top);
    for (pRow : EnemyRows) {
        byte n;
        n = pRow->memberCount;
        if (pRow->memberCount != 0) {
            ...
            repeat(ColumnCount) {
                if ((bits & mask) != 0) {
                    --n;
                    if (leftLive) {
                        wUpper = WriteVram(wUpper, pChars[4]);
                        wLower = WriteVram(wLower, pChars[5]);
                    }
                    else {
                        wUpper = WriteVram(wUpper, pChars[0]);
                        wLower = WriteVram(wLower, pChars[1]);
                    }
                    wUpper = WriteVram(wUpper, pChars[6]);
                    wLower = WriteVram(wLower, pChars[7]);
                    leftLive = true;
                }
                else {
                    if (leftLive) {
                        wUpper = WriteVram(wUpper, pChars[2]);
                        wLower = WriteVram(wLower, pChars[3]);
                    }
                    else {
                        wUpper = WriteVram(wUpper, Char_Space);
                        wLower = WriteVram(wLower, Char_Space);
                    }
                    wUpper = WriteVram(wUpper, Char_Space);
                    wLower = WriteVram(wLower, Char_Space);
                    leftLive = false;
