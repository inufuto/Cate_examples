        ...
        if (n != 0) {
            ...
            repeat (ColumnCount) {
                ...
                if (x >= LeftSpace) {
                    ...
                    if (type != 0) {
                        if (leftType != 0) {
                            pUpper = WriteVram(pUpper, CharsO30[UfoRowCenterXd]);
                            pLower = WriteVram(pLower, CharsO74[UfoRowCenterXd]); 
                        }
                        else {
                            pUpper = WriteVram(pUpper, CharsOS0[UfoRowCenterXd]);
                            pLower = WriteVram(pLower, CharsOS4[UfoRowCenterXd]);
                        }
                        ptr<byte> pc = CharsO03 + UfoRowCenterXd;
                        repeat (3) {
                            pUpper = WriteVram(pUpper, *pc);
                            pc += CoordRate;
                        }
                        ...
                        if (leftType != 0 && UfoRowCenterXd != 0) {
                            WriteVram(pUpper, CharsO3S[UfoRowCenterXd]);
                            WriteVram(pLower, CharsO7S[UfoRowCenterXd]); 
                        }
                        pUpper += UnitWidth * VramStep;
                        pLower += UnitWidth * VramStep;
                        leftType = 0;
                    }
                }
                x += UnitWidth;
