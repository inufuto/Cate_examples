...
constexpr byte FixedX = WindowWidth / 2 * 8 - 8;
constexpr byte FixedY = WindowHeight / 2 * 8 - 8;
...
{
    UpdateBasePosition();
    ShowSprite(Sprite_Fighter, FixedX, FixedY, Pattern_Fighter + FighterDirection);
    ShowSprite(Sprite_MapPoint, 8 * 14 - 1 + (FighterX >> 1), 8 * 10 - 1 + (FighterY >> 1), 
