...
void InitSolver(byte x, byte y) 
{
    ...
    ButtonOn = false;
    xx = SolverX - 8;
    ShowSprite(Sprite, xx, SolverY - 8, pDirection->pattern);
    SetScrollTarget(xx);
    HScroll = ScrollTerget;
}
...
static bool Move(ptr<Direction> pNewDirection) 
{
    ...
    if (TestMap2(newX, newY)) {
        ...
        x = SolverX - 8;
        ShowSprite(Sprite, x, SolverY - 8, pDirection->pattern + seq);
...
void EndSolver()
{
    ...
    SetScrollTarget(x);
    repeat(20) {
        ShowSprite(Sprite, x, SolverY - 8, RotationPatterns[i & 3]);
        ++i;
        // EraseSprites();
        // DrawSprites();
        // VVramToVram();
