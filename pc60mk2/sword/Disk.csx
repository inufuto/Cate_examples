...
private const int TrackCount = 40;
private const int SectorCount = 16;
private const int OffsetCount = 164;
private const int IplTrackNumber = 0;
...
private static void ReadIpl()
{
    ...
    {
        Tracks[IplTrackNumber].Sectors[index].SetBytes(bytes);
        ++index;
    }
}
...
private static void ReadProgram(string name)
{
    ...
    var trackNumber = IplTrackNumber;
    var sectorIndex = 1;
    var bytes = new byte[Sector.MaxSize];
    while (stream.Read(bytes) > 0)
    {
