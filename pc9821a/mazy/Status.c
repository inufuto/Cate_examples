...
void PrintStatus() 
{
    ...
    {
        constexpr ptr<byte> RemainLeft = Vram + VramRowSize * 26 + (VVramWidth + 1) * VramStep;
        word vram;
        byte count;
        vram = RemainLeft;
...
void PrintHeldKnives()
{
    ...
    vram = Vram + VramRowSize * 24 + (VVramWidth + 1) * VramStep;
    i = 0;
...
void Title()
{
    ...
    Put2C(Vram + VramRowSize * 8 + 9 * VramStep, Char_Man);
    Put2C(Vram + VramRowSize * 10 + 9 * VramStep, Char_Chaser);
    Put2C(Vram + VramRowSize * 12 + 9 * VramStep, Char_Disturber);
    Put2C(Vram + VramRowSize * 14 + 9 * VramStep, Char_Goal);
    
    PrintStatus(); 
    PrintS(Vram + VramRowSize * 5 + 8 * VramStep, "--- CHARACTER ---");
    PrintS(Vram + VramRowSize * 8 + 14 * VramStep, "MAN");
    PrintS(Vram + VramRowSize * 10 + 14 * VramStep, "CHASER");
    PrintS(Vram + VramRowSize * 12 + 14 * VramStep, "DISTURBER");
    PrintS(Vram + VramRowSize * 14 + 14 * VramStep, "EXIT");

    PrintS(Vram + VramRowSize * 24 + (VVramWidth - 14) * VramStep / 2, "PUSH SPACE KEY");
    PrintS(Vram + VramRowSize * 25 + (VVramWidth - 14) * VramStep / 2, "OR GRPH KEY");
    PrintS(Vram + VramRowSize * 27 + (VVramWidth - 12) * VramStep, "INUFUTO 2026");
    // {
    //     byte c;
    //     word v;
    //     v = Vram+32*6;
