...
{
    static const byte[] Colors = { 0x91, 0xd1, 0xa1 };
    ShowSprite2(pEnemy, pEnemy->pattern + Pattern_SkyEnemy, Colors[pEnemy->type]);
}

static void End(ptr<SkyEnemy> pEnemy)
{
    pEnemy->y = InvalidY;
    HideSprite2(pEnemy->sprite);
}
    ...
    case Type_Smart: 
        {
            ...
            pEnemy->pattern = (pEnemy->direction << 1) + (Pattern_SkyEnemyB - Pattern_SkyEnemy);
