...
private const byte TypeByte = 0x30;
private const int TrackCount = 70;
...
{
    using var stream = new FileStream(name + "_d.bin", FileMode.Open);
...
ReadLoader();
ReadProgram(name);
using (var stream = new FileStream(name + ".1dd", FileMode.Create))
{
