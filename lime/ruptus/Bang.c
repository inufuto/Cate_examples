...
void UpdateBangs()
{
    ...
    for (pBang : Bangs) {
        ...
        if (mode == Status_Large_Large) {
            sprite = Locate(pBang->x - HalfSize, pBang->y - HalfSize, sprite, Pattern_Bang + 1);
            sprite = Locate(pBang->x + HalfSize, pBang->y - HalfSize, sprite, Pattern_Bang + 2);
            sprite = Locate(pBang->x - HalfSize, pBang->y + HalfSize, sprite, Pattern_Bang + 3);
            sprite = Locate(pBang->x + HalfSize, pBang->y + HalfSize, sprite, Pattern_Bang + 4);
        }
        else {
