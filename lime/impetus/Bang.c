...
void UpdateBangs()
{
    ...
    for (pBang : Bangs) {
        ...
        if (mode == Status_Large_Large) {
            sprite = Show(pBang->x - 1, pBang->y - 1, sprite, Pattern_LargeBang + 0, Color);
            sprite = Show(pBang->x + 1, pBang->y - 1, sprite, Pattern_LargeBang + 1, Color);
            sprite = Show(pBang->x - 1, pBang->y + 1, sprite, Pattern_LargeBang + 2, Color);
            sprite = Show(pBang->x + 1, pBang->y + 1, sprite, Pattern_LargeBang + 3, Color);
        }
        else {
            sprite = Show(pBang->x, pBang->y, sprite, Pattern_SmallBang, Color);
        }
