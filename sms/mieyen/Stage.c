...
void InitTrying()
{
    ...
    CopyMemory(CellMap, pStage->bytes, ColumnCount / ColumnsPerByte * RowCount);
    {
                ...
                repeat (ColumnCount / ColumnsPerByte) {
                    ...
                    repeat (ColumnsPerByte) {
                        if ((cell & Cell_BottomWall) != 0) {
                            Put(vram + VramRowSize * 2, Char_Wall | Wall_All);
                            Put(vram + VramRowSize * 2 + 1 * VramStep, Char_Wall | Wall_All);
                        }
                        vram += 2 * VramStep;
        ...
        }
        {
            ...
            for (y = 0; y < VramHeight - StatusHeight; ++y) {
                ...
                for (x = 0; x < VramWidth; ++x) {
                    ...
                    // byte old = c;
                    if (x > 0 && ReadVram(vram - 1 * VramStep) != Char_Space) {
                        c &= ~Wall_Left;
                    }
                    if (ReadVram(vram + 1 * VramStep) != Char_Space) {
                        c &= ~Wall_Right;
                    }
                    if (y > 0 && ReadVram(vram - VramRowSize) != Char_Space) {
                        c &= ~Wall_Top;
                    }
                    if (y < VramHeight - StatusHeight - 1 && ReadVram(vram + VramRowSize) != Char_Space) {
                        c &= ~Wall_Bottom;
                    }
                    vram = Put(vram, c);
                }
        ...
        {
            FoodCount = 0;
            word vram = Vram + VramRowSize * (StatusHeight + 1) + 1 * VramStep;
            ptr<byte> pCell = CellMap;
            repeat (RowCount) {
                repeat (ColumnCount / ColumnsPerByte) {
                    byte cell = *pCell; ++pCell;
