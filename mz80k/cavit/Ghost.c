...
static void Draw(ptr<Movable> pGhost)
{
    static const byte[] Chars = {
        0x4a, 0x4a, 0x43, 0x56, 0x4a, 0x4a, 0x42, 0x43,
	    0x4a, 0x4a, 0x42, 0x56, 0x43, 0x43, 0x42, 0x56,
	    0x48, 0x48, 0x32, 0x76, 0x48, 0x48, 0x77, 0x33,
	    0x48, 0x48, 0x77, 0x76, 0x72, 0x73, 0x77, 0x76,
    };
    byte status, c;
    status = pGhost->status;
    c = ((status & Movable_Direction_Mask) << 1);
    if ((status & Ghost_Status_Throgh) != 0) {
        c += 4 * 4;
    }
    ShowSprite(pGhost, Chars + c);
}
