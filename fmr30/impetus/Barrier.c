...
constexpr byte Count = (Sprite_Item - Sprite_BarrierHead) / 2;
...
{
    ShowSprite(pBarrier->sprite, pBarrier->left, pBarrier->y, Pattern_BarrierHead);
    ShowSprite(pBarrier->sprite + 1, pBarrier->right, pBarrier->y, Pattern_BarrierHead);
}
...
void DrawBarriers()
{
    ptr<Barrier> pBarrier;
    for (pBarrier : Barriers) {
        if (pBarrier->y < StageHeight * CoordRate) {
            DrawBarrierLine(
                (pBarrier->left >> CoordShift) + 1,
                pBarrier->y,
                pBarrier->length
            );
            pBarrier->oldY = pBarrier->y >> CoordShift;
        }
    }
}
