...
static ptr<byte> FillTile2(ptr<byte> pBackground, ptr<byte> pChars)
{
    ...
    return pBackground + VVramWidth - ColumnWidth;
}
...
static ptr<byte> FillTile4(ptr<byte> pBackground, ptr<byte> pChars)
{
    repeat (FloorHeight - 2) {
        ...
        pBackground += VVramWidth - ColumnWidth;
    }
    return pBackground;
}
...
void FillBackground()
{
    ...
    ptr<byte> pMap, pMapU, pMapL, pBackground;
    word vram;
    byte row;
    if ((topRow & 0x80) != 0) {
        pMap = StageMap;
    ...
    repeat (VisibleFloorCount) {
        ...
        repeat (MapWidth) {
            ...
            if ((row & 0x80) != 0) {
                ...
                upper = row != 0 ? *pMapU : CellType_Space;
            }
            ++pMap; ++pMapU; ++pMapL;
            repeat (4) {
                ...
                pBackground += ColumnWidth - VVramWidth * FloorHeight;
                middle >>= 2;
                lower >>= 2;
                upper >>= 2;
            }
        ...
        pBackground += VVramWidth * (FloorHeight - 1);
        ++row;
    }
}
...
void DrawBackground()
{
    ...
    // charOffset ^= Char_Alter - Char_BackGround;
    // UpdateBackgroundChars();
    BackgroundToVVram();
    // SwitchChars();
}
