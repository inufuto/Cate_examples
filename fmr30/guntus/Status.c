...
void PrintStatus()
{
    ...
    PrintByteNumber2(Vram + VramRowSize * 7 + VramWidth - 2 * VramStep, CurrentStage + 1);
    PrintRemain();
    PrintScore();
}
...
void PrintGameOver()
{
    ...
    PrintS(Vram + VramRowSize * (WindowHeight / 2) + Left * VramStep, "GAME OVER");
}
...
void Title()
{
    ...
    PrintS(Vram + VramRowSize * 24 + (WindowWidth - 12) * VramStep, "INUFUTO 2025");
    // {
    //     byte c;
    //     word v;
    //     EnemyRowLeft = 0;
