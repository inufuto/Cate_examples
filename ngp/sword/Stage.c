...
void InitStage()
{
    ...
    PrintStatus();
    {
        ...
        while (row < RowCount) {
            byte column = 0;
            repeat ((ColumnCount + ColumnsPerByte - 1) / ColumnsPerByte) {
...
void SetCell(byte column, byte row, byte cell)
{
    ...
    word vram = VramBack + VramOffset(column << GridShift, (row << GridShift) + StatusHeight);
    if (cell == Cell_Space) {
        Erase2(vram);
    }
    else {
        Put2C(vram, Char_Wall + ((cell - 1) << 2));
    }
}
