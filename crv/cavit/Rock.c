...
{
    Put2CXY(pFixedRock->x, pFixedRock->y + StageTop, Char_Rock);
...
{
    ShowSprite(pFallingRock, Pattern_Rock, Color);
}
    ...
    for (pFallingRock: FallingRocks) {
        ...
        if (CanChangeDirection(pFallingRock)) {
            ...
            if ((below & Terrain_Mask) != Terrain_Space) {
                ...
                EndFalling(pFallingRock->x >> CoordShift, pFallingRock->y >> CoordShift);
                continue;
            }
...
{
    return AdjacentOtherInArray(FallingRocks, MaxMovableCount, pMovable, x, y);
}