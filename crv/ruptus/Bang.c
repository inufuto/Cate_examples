...
static byte Locate(byte x, byte y, byte sprite, byte pattern, byte color)
{
    ...
    if (x < (VVramWidth -Size) * 2 && y < (VVramHeight - Size) * 2) {
        ShowSprite(sprite, x << 2, y << 2, pattern, color);
        return sprite + 1;
    }
end:
    return sprite;
...
void UpdateBangs()
{
    ...
    for (pBang : Bangs) {
        ...
        if (mode == Status_Large_Large) {
            sprite = Locate(pBang->x - Size, pBang->y - Size, sprite, Pattern_Bang + 1, color);
            sprite = Locate(pBang->x + Size, pBang->y - Size, sprite, Pattern_Bang + 2, color);
            sprite = Locate(pBang->x - Size, pBang->y + Size, sprite, Pattern_Bang + 3, color);
            sprite = Locate(pBang->x + Size, pBang->y + Size, sprite, Pattern_Bang + 4, color);
        }
        else {
            sprite = Locate(pBang->x, pBang->y, sprite, Pattern_Bang, color);
        }
