...
var address = 0x0000;
var bootBytes = new byte[0x80];
bootBytes[0] = 0xaa;
bootBytes[1] = 0x0b;
bootBytes[2] = (byte)(address % 0x100);
bootBytes[3] = (byte)(address / 0x100);
bootBytes[4] = Segment % 0x100;
bootBytes[5] = Segment / 0x100;
bootBytes[6] = (byte)(address % 0x100);
bootBytes[7] = (byte)(address / 0x100);
...
ReadProgram(name);
{
    ...
    while (Tracks[trackNumber].Sectors[sectorIndex].Size() > 0)
    {
        ...
        var blockCount = count - sectorIndex;
        ...
        bootBytes[bootIndex++] = (byte)(0x40 | blockCount);
        ++trackNumber;
        address += blockCount * 0x100;
        sectorIndex = 0;
        if (bootIndex == 30) {
            bootBytes[bootIndex++] = 0xff;
            bootBytes[bootIndex++] = 0x00;
            bootBytes[bootIndex++] = (byte)(address % 0x100);
            bootBytes[bootIndex++] = (byte)(address / 0x100);
            bootBytes[bootIndex++] = Segment % 0x100;
            bootBytes[bootIndex++] = Segment / 0x100;
        }
    }
    bootBytes[bootIndex++] = 0xff;
