...
static void Draw(ptr<byte> pPage, ptr<Block> pBlock)
{
    ...
    if (y < WindowHeight) {
        ptr<byte> pVram = pPage + VramRowSize * StatusHeight + VramOffset(pBlock->x, y);
        ptr<byte> pChars = Chars + (word)(((dotOffset << 1) + dotOffset) << 1);
        repeat (3) {
            if (y < WindowHeight) {
                repeat (2) {
                    pVram = WriteVram(pVram, *pChars);
                    ++pChars;
                }
...
static void Hide(ptr<Block> pBlock)
{
    ...
    pVram = pNextPage + VramRowSize * StatusHeight + VramOffset(pBlock->x, y);
    repeat (3) {
        if (y < WindowHeight) {
            repeat (2) {
