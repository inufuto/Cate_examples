...
constexpr byte FixedX = (WindowWidth / 2 * 8 - 8) / 4;
constexpr byte FixedY = (WindowHeight / 2 * 8 - 8) / 4;
...
{
    UpdateBasePosition();
    ShowSprite(Sprite_Fighter, FixedX * 4, FixedY * 4, Pattern_Fighter + FighterDirection);
    {
        ShowSprite(
            Sprite_MapPoint, 
            8 * WindowWidth - 1 + (FighterX >> 1), 
