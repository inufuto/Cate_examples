...
void PrintScore()
{
    ...
    pVram = PrintNumber5(Vram + VramRowSize * 1 + WindowWidth * VramStep, Score);
    PrintC(pVram, '0');

    pVram = PrintNumber5(Vram + VramRowSize * 5 + WindowWidth * VramStep, HiScore);
    PrintC(pVram, '0');
}
...
{
    ptr<byte> pVram;
    pVram = Vram + VramRowSize * 23 + (WindowWidth + 1) * VramStep;
    if (RemainCount > 1) {
        byte i;
        i = RemainCount - 1;
        if (i > 2) {
...
void Title()
{
    ...
    PrintS(Vram + VramRowSize * 21 + 9 * VramStep, "Z:START");
    PrintS(Vram + VramRowSize * 22 + 9 * VramStep, "X:CONTINUE");
    PrintS(Vram + VramRowSize * 24 + 14 * VramStep, "INUFUTO 2026");
    // {
    //     byte c;
    //     ptr<byte> pVram;
    //     pVram = Vram;
