...
void InitTrying()
{
    ...
    {
        ptr<byte> pCell = CellMap;
        ptr<byte> pVVram = VVram - VVramWidth * 2;
        byte y = -2;
        repeat (FloorCount) {
            repeat (ColumnCount) {
                byte cell = *pCell; ++pCell;
                pVVram = DrawCell(pVVram, cell, y);
            }
            y += FloorHeight;
            pVVram += VVramWidth * FloorHeight - ColumnCount * ColumnWidth;
        }
...
void DrawCellCR(byte column, byte row)
{
    ...
    DrawCell(VVramPtr(column << CellShift, y), b, y);
}