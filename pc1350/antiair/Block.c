...
void InitBlocks()
{
    ...
    FillMemory(Ground, VVramWidth, 0);
}
...
{
    ptr<byte> pGround;
    StartLargeBang((bombX + 1) << CoordShift, (VVramHeight - 1) * CoordRate);
    // Sound_LargeBang();
    HitCannon((bombX - 1) << CoordShift, ((VVramHeight - 2) * CoordRate), 4 * CoordRate);
