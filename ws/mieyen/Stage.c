...
void InitTrying()
{
    ...
    CopyMemory(CellMap, pStage->bytes, ColumnCount / ColumnsPerByte * RowCount);
    {
        ...
        {
            ptr<byte> pVram = VramBack + VramRowSize * StatusHeight;
            repeat (ColumnWidth * ColumnCount + 1) {
                pVram = Put(pVram, Char_Wall | Wall_All);
            }
            ptr<byte> pCell = CellMap;
            pVram += VramRowSize - (ColumnWidth * ColumnCount + 1) * VramStep;
            repeat (RowCount) {
                Put(pVram, Char_Wall | Wall_All);
                Put(pVram +VramRowSize, Char_Wall | Wall_All);
                Put(pVram +VramRowSize * 2, Char_Wall | Wall_All);
                pVram += VramStep;
                repeat (ColumnCount / ColumnsPerByte) {
                    ...
                    repeat (ColumnsPerByte) {
                        if ((cell & Cell_BottomWall) != 0) {
                            Put(pVram + VramRowSize * 2, Char_Wall | Wall_All);
                            Put(pVram + VramRowSize * 2 + VramStep, Char_Wall | Wall_All);
                        }
                        pVram += 2 * VramStep;
        ...
        }
        {
            ...
            for (y = 0; y < VramHeight - StatusHeight; ++y) {
                ...
                for (x = 0; x < ColumnWidth * ColumnCount + 1; ++x) {
                    byte c = ReadVram(pVram);
                    byte old = c;
                    if (x > 0 && ReadVram(pVram - VramStep) != Char_Space) {
                        c &= ~Wall_Left;
                    }
                    if (ReadVram(pVram + VramStep) != Char_Space) {
                        c &= ~Wall_Right;
                    }
                    if (y > 0 && ReadVram(pVram - VramRowSize) != Char_Space) {
                        c &= ~Wall_Top;
                    }
                    if (y < VramHeight - StatusHeight - 1 && ReadVram(pVram +VramRowSize) != Char_Space) {
                        c &= ~Wall_Bottom;
                    }
                    pVram = Put(pVram, c);
                }
        ...
        {
            FoodCount = 0;
            ptr<byte> pVram = VramBack + VramRowSize * (StatusHeight + 1) + 1 * VramStep;
            ptr<byte> pCell = CellMap;
            repeat (RowCount) {
                repeat (ColumnCount / ColumnsPerByte) {
                    byte cell = *pCell; ++pCell;
...
void SetCell(byte column, byte row, byte cell)
{
    ...
    ptr<byte> pVram = VramBack + VramOffset(ToCoord(column), ToCoord(row) + StatusHeight);
    if ((cell & Cell_Dot) != 0) {
        Put2C(pVram, Char_Dot);
    }
