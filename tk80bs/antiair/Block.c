...
const byte[] BlockChars = {
    0x9e, 0x9f, 0xa0, 0xa1, 0xb8, 0xb8, 0xb5, 0xb1,
	0xc2, 0xa3, 0xc4, 0xc5,
};
...
void MoveBlocks()
{
    ...
    for (pBlock: FallingBlocks) {
        if (pBlock->flags != 0) {
            if ((pBlock->y & CoordMask) == 0) {
                byte x, y;
                x = pBlock->x >> CoordShift;
                y = pBlock->y >> CoordShift;
                if (y >= WindowHeight - 2) {
                    byte type;
                    ...
                    if (HitCannon(pBlock->x, pBlock->y, 2 * CoordRate)) {
                        Hide(pBlock);
                        goto next;
                    }
                }
