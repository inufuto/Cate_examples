...
void UpdateBangs()
{
    ...
    for (pBang : Bangs) {
        ...
        if (mode == Status_Large_Large) {
            sprite = Show(pBang->x - Size, pBang->y - Size, sprite, GChar_Bang + 8, 2);
            sprite = Show(pBang->x + Size, pBang->y - Size, sprite, GChar_Bang + 24, 2);
            sprite = Show(pBang->x - Size, pBang->y + Size, sprite, GChar_Bang + 16, 3);
            sprite = Show(pBang->x + Size, pBang->y + Size, sprite, GChar_Bang + 32, 3);
        }
        else {
            sprite = Show(pBang->x, pBang->y, sprite, GChar_Bang, 1);
        }
