...
ptr<byte> InitMovables(ptr<Movable> pMovable, byte maxCount, byte sprite, ptr<byte> pStage)
{
    byte count, i;
    ...
    while (i < count) {
        ...
        pMovable->x = MovableX(b);
        pMovable->y = MovableY(b);
        pMovable->status = Movable_Status_Live;
        pMovable->sprite = sprite;
        ++sprite;
        ++pMovable;
        ++i;
    }
    while (i < maxCount) {
        pMovable->status = Movable_Status_None;
        ++pMovable;
        ++i;
    }
    return pStage;
