...
static void Locate(ptr<Enemy> pEnemy)
{
    byte x, y;
    x = OffsetX(pEnemy->x);
    if (x + WindowWidth / 2 >= WindowWidth * 2 + WindowWidth) goto end;
    y = OffsetY(pEnemy->y);
    if (y + WindowWidth / 2 >= WindowHeight * 2 + WindowWidth) goto end;
    x -= HalfSize;
    y -= HalfSize;
    if (x < (WindowWidth - HalfSize) * 2 && y < (WindowHeight - HalfSize) * 2) {
        ShowSprite(pEnemy->sprite, x, y, Chars[pEnemy->direction & 7]);
    }
    else {
...
static bool Hit(ptr<Enemy> pEnemy)
{
    ...
    // if (HitBarrier(pEnemy->x, pEnemy->y, HalfSize)) {
    //     goto end;
    // }
    // if (HitFort(pEnemy->x, pEnemy->y, HalfSize)) {
    //     goto end;
    // }
    return false;
}
