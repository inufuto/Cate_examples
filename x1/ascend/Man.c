...
{
    byte pattern;
    if (Man.pattern != GChar_Man_Climb) {
        if (Man.dx == 0) {
            pattern = Man.pattern;
        }
        else {
            constexpr byte Horizontal = GChar_Man_Left0 - GChar_Man_Left;
            byte seq;
