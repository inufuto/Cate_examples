        ...
        if ((pFire->status & Actor_Live) != 0) {
            ...
            if (IsOnFloor(pFire->_.y)) {
                if ((pFire->_.x & 1) == 0 && (
                    ...
                    if (pFire->dx < 0) {
                        if (!CanMove(pFire, -1)) {
                            pFire->dx = 1;
                            pFire->pattern = GChar_Fire_Right;
                        }
                        else {
                            pFire->pattern = GChar_Fire_Left;
                        }
                    }
                    else if (pFire->dx > 0) {
                        if (!CanMove(pFire, 1)) {
            ...
            if (pFire->_.y > topY && pFire->_.y - topY >= WindowHeight) {
                ...
                if ((pFire->status & Item_1Up) != 0) {
                    ShowSprite(&pFire->_, GChar_Item_1Up);
                }
                else if ((pFire->status & Item_Power) != 0) {
                    ShowSprite(&pFire->_, GChar_Item_Power);
                }
                else {
                    ShowEnemy(pFire);
                }
