...
cseg
PaletteValues:
	defb	$00, $9b, $39, $7f, $ff, $bf, $1f, $07
	defb	$01, $37, $5a, $39, $67, $dc, $3f, $46
ColorTable:
defb 16,5 ; Logo
defb 0
...
DrawSprite_: public DrawSprite_
    pha
        ...
        sty BLT_VX
        lda DrawSprite_y | clc|adc #VramTopY | sta BLT_VY
...
dseg
DrawBarrierLine_@Param1: public DrawBarrierLine_@Param1
DrawBarrierLine_y:
    defb 0
DrawBarrierLine_@Param2: public DrawBarrierLine_@Param2
DrawBarrierLine_length:
    defb 0
cseg
DrawBarrierLine_: public DrawBarrierLine_
    pha
        ...
        sty BLT_VX
        lda DrawBarrierLine_y | clc|adc #VramTopY+1 | sta BLT_VY
        lda DrawBarrierLine_length | asl a | asl a | sta BLT_WIDTH
        lda #1 | sta BLT_HEIGHT
        lda #not $1f | sta BLT_COLOR
        lda #1 | sta <BltBusy | sta BLT_START
        do
            lda <BltBusy
        while ne | wend
...
ext FortY_, FortLife_

FortX equ (VVramWidth-FortWidth)/2*CharWidth
...
DrawFort_: public DrawFort_
    pha
        ...
        if ne
            lda FortY_
            if ne
                sec|sbc #CharHeight*FortHeight
                if cc
                                    _deb: public _deb
                    eor #$ff | inc a | adc #VramHeight+FortPatternY | sta BLT_GY
                    lda #VramTopY | sta BLT_VY
                    lda FortY_ | sta BLT_HEIGHT
                else
                    clc|adc #VramTopY | sta BLT_VY
