...
cseg
PaletteValues:
	defb	$00, $9b, $39, $7f, $ff, $97, $1f, $07
	defb	$01, $e5, $34, $5d, $fa, $bb, $4f, $67
ColorTable:
defb 1,5 ; Space
defb 1,5 ; Wall
defb 4,7 ; Goal
...
PresentBackground_: public PresentBackground_
    pha
        ...
        lda #VramVisibleHeight | sta BLT_HEIGHT
...
EraseRightEnd_: public EraseRightEnd_
    pha
        ...
        lda #0 | sta BLT_VX
        lda #VramTopY+CharHeight*VVramHeight | sta BLT_VY
        lda #CharWidth*VVramWidth | sta BLT_WIDTH
        lda #VramHeight-VramTopY+CharHeight*VVramHeight | sta BLT_HEIGHT
        lda #not 0 | sta BLT_COLOR
        lda #1 | sta <BltBusy | sta BLT_START
        do
            lda <BltBusy
...
cseg
CharTable:
    defb " -0123456789:ABCDEFGHIMNORSTUVX"
