...
{
    byte seq = pMovable->status & Movable_Pattern;
    pattern += seq >> 1;
...
bool IsOnGrid(ptr<Movable> pMovable)
{
    ...
    if ((pMovable->x & 1) != 0) return false;
    if ((pMovable->y & 1) != 0) return false;
    return true;
}
...
bool IsNear(ptr<Movable> p1, ptr<Movable> p2)
{
    byte x1 = p1->x;
    byte x2 = p2->x;
    if (x1 + HitRange >= x2 && x2 + HitRange >= x1) {
        byte y1 = p1->y;
        byte y2 = p2->y;
        return y1 + HitRange >= y2 && y2 + HitRange >= y1;
    }
    return false;
}
