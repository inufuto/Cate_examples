private const string LoaderName = "Loader2DD.bin";

private const int NameSize = 17;

private const byte Protected = 0x00;
private const byte Type2DD = 0x10;
private const int TrackCount = 40 * 4;
private const int SectorCount = 9;
...
class Sector
{
    ...
    private const byte SizeType = 0x02;
    ...
    public void WritePlain(Stream stream)
    {
        stream.Write(bytes.ToArray());
        WriteRepeat(stream, 0, MaxSize - bytes.Count);
    }
...
ReadLoader();
ReadProgram(name);
using (var stream = new FileStream(name + "_2dd.d88", FileMode.Create))
{
...
using (var stream = new FileStream(name + "_2dd.img", FileMode.Create))
{
    for (var i = 0; i < TrackCount; ++i)
    {
        Tracks[i].WritePlain(stream);
    }
}
