...
static void Locate(ptr<Enemy> pEnemy)
{
    byte x, y;
    x = OffsetX(pEnemy->x);
    if (x + WindowWidth / 4 >= WindowWidth + WindowWidth / 2) goto end;
    y = OffsetY(pEnemy->y);
    if (y + WindowWidth / 4 >= WindowHeight + WindowWidth / 2) goto end;
    x -= HalfSize;
    y -= HalfSize;
